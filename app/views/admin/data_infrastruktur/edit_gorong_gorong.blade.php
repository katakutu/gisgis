	...
	<div id="content_body">
		...
		<div id="kanan">
			...
			{{Form::open(array('url' => 'admin/data_infrastruktur/ajax/upload_foto', 'method'=>'post', 'enctype'=>'multipart/form-data', 'id'=>'form_upload_foto'))}}
				<input type="hidden" name="id" value="{{$gorong_gorong->id}}">
				<input type="hidden" name="kategori" value="gorong_gorong">
			...
			<form action="" method="post" enctype="multipart/form-data">
				...
				<select name="kecamatan" id="">
					...
					<option value="{{$value->id}}" @if($value->id == $gorong_gorong->kecamatan_id) selected="selected" @endif>{{$value->nama_kecamatan}}</option>
					@endforeach
				</select><br>
				...
				<select name="id_ruas_jalan" id="">
					...
					<option value="{{$value->id}}" @if($value->id == $gorong_gorong->id_ruas_jalan) selected="selected" @endif>{{$value->nama_ruas}}</option>
					@endforeach
				</select><br>
				<label for="">Bahan Bangunan Atas</label>
				<input type="text" name="bahan_bangunan_atas" value="{{$gorong_gorong->bahan_bangunan_atas}}"><br>
				<label for="">Kondisi Bangunan Atas</label>
				<input type="text" name="kondisi_bangunan_atas" value="{{$gorong_gorong->kondisi_bangunan_atas}}"><br>
				<label for="">No GPS</label>
				<input type="text" name="no_gps" value="{{$gorong_gorong->no_gps}}"><br>

				<?php
				$koordinat = explode(' ', $gorong_gorong->koordinat_gps);
				?>
				...
				<input type="text" name="panjang" placeholder="contoh(9,2)" value="{{$gorong_gorong->panjang}}"><br>
				<label for="">Lebar (m)</label>
				<input type="text" name="lebar" placeholder="contoh(6,2)" value="{{$gorong_gorong->lebar}}"><br>
