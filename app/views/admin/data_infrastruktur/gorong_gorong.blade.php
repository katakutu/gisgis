...
@section('content')
	...
	<div id="content_body">
		...
		<div id="kanan">
			{{HTML::link('admin/data_infrastruktur/gorong_gorong/tambah', 'Tambah Gorong-Gorong', array('id'=>'tambah'))}}
			<table cellspacing="0">
				<tr>
				...
				<?php $loop = 1; ?>
				@foreach($gorong_gorong as $key=>$value)
				<tr>
					<td>{{$loop}}</td>
					...
					<td>{{$value->lebar}} m</td>
					<td>{{HTML::link('admin/data_infrastruktur/gorong_gorong/edit/'.$value->id, 'Edit')}} | {{HTML::link('admin/data_infrastruktur/gorong_gorong/delete/'.$value->id, 'Delete')}}</td>
				</tr>
				<?php $loop++ ?>
				@endforeach
			</table>
